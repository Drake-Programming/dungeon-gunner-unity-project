...
public static class AStar
{
    ...
    public static Stack<Vector3> BuildPath(Room room, Vector3Int startGridPosition, Vector3Int endGridPosition)
    {
        ...
        // Create open heap and closed hashset
        BinaryHeap<Node> openNodeList = new BinaryHeap<Node>(gridNodes.MaxSize);
    ...
    private static Node FindShortestPath(Node startNode, Node targetNode, GridNodes gridNodes, 
        BinaryHeap<Node> openNodeList, HashSet<Node> closedNodeHashSet, InstantiatedRoom instantiatedRoom)
    {
        ...
        while (openNodeList.Count > 0)
        {
            ...
            Node currentNode = openNodeList.RemoveFirstItem();
        ...
        for (int i = -1; i <= 1; i++)
        {
            for (int j = -1; j <= 1; j++)
            {
                ...
                if (validNeighbourNode != null)
                {
                    ...
                    // Unwalkable paths havea value of 0. Default movement penality is set in settings and applies to other grid squares
    ...
    private static int GetDistance(Node nodeA, Node nodeB)
    {
        ...
        if (distanceX > distanceY)
            return 14 * distanceY + 10 * (distanceX - distanceY); // 10 used instead of 1, and 14 is a pythagoras approximation SQRT(10*10) - to avoid using floats
    ...
    /// cloasedNoseHashSet, and instantiated room. Returns null if the node isn't valid
    /// </summary>
